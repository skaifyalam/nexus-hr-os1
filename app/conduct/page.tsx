...
export default async function ConductPage() {
  ...
  const [{ data: conduct }, { data: exits }, { data: empFields }, { data: empSection }, { data: remobs }] = await Promise.all([
    ...
    supabase.from('remobilizations').select('*').eq('company_id', companyId).order('created_at', { ascending: false }),
  ]);
  ...
  return (
    <Shell current="/conduct" profile={profile} sections={sections} companyId={companyId}>
      <ConductClient initialConduct={conduct || []} initialExits={exits || []} initialRemobs={remobs || []} employees={employees} empFields={empFields || []} activeConfig={empSection || null} companyId={companyId} userEmail={user?.email || ''} />
    </Shell>
  );
}
