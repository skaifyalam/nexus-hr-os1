...
export default async function ConductPage() {
  ...
  const [{ data: conduct }, { data: exits }, { data: empFields }] = await Promise.all([
  ...
  return (
    <Shell current="/conduct" profile={profile} sections={sections} companyId={companyId}>
      <ConductClient initialConduct={conduct || []} initialExits={exits || []} employees={employees} empFields={empFields || []} companyId={companyId} userEmail={user?.email || ''} />
    </Shell>
  );
}
