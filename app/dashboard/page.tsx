...
export default async function DashboardPage() {
  ...
  const [{ data: employees }, { data: requisitions }, { data: leaves }] = await Promise.all([
  ...
  const expiring = emp.filter(e => (e.iqama_expiry && daysUntil(e.iqama_expiry) <= 60) || (e.passport_expiry && daysUntil(e.passport_expiry) <= 60)).length;
  ...
  return (
    <Shell current="/dashboard" profile={profile} modules={modules} customSections={customSections}>
      <div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {stats.map((s, i) => (
            <div key={i} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-3 ${s.light}`}><s.icon size={16} /></div>
        ...
        {emp.length === 0 && (
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center">
            <p className="text-sm text-slate-500">No employees yet. Go to Employees to add your first one or import an Excel file.</p>
          </div>
        )}
      </div>
    </Shell>
