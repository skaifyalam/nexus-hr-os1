...
export default function ReportsClient({ initialReports, companyId, userEmail }: {
  ...
  const exportFormat = async (format: string) => {
    if (!current?.report_id && !current?.id) {
      // fallback to plain txt if no report_id saved
    ...
    try {
      const res = await fetch('/api/export-report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ report_id: current.report_id || current.id, format }),
      });
      const data = await res.json();
      if (data.error) { setError(data.error); setExporting(false); return; }
        ...
        <div className="col-span-2">
          {loading && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
              ...
              <p className="text-sm font-medium text-slate-600">Naibus AI is generating your report…</p>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50 rounded-t-2xl">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-slate-500">Generated by Naibus AI</span>
                ...
                <div className="relative">
                  <button onClick={() => setExportOpen(!exportOpen)}
                    ...
                    <Download size={12} />{exporting ? 'Exporting…' : 'Download'}
                  </button>
                  {exportOpen && (
                    <div className="absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg py-1 w-44 z-10">
                      <button onClick={() => exportFormat('excel')} className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50">📊 Excel (.csv)</button>
                      <button onClick={() => exportFormat('word')} className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50">📄 Word (.doc)</button>
                      <button onClick={() => exportFormat('ppt')} className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50">📑 PowerPoint slides</button>
                      <button onClick={downloadTxt} className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50">📝 Plain text</button>
                    </div>
                  )}
                </div>
              </div>
